...
try:
    model = joblib.load('exoplanet_xgboost_model.pkl')
...
def create_summary_report(results_df: pd.DataFrame, processing_time: float) -> str:
...
Model: XGBoost Classifier (Accuracy: 99.27%)
...
async def health_check():
    ...
    return {
        ...
        "classes": label_encoder.classes_.tolist() if model else []
    }
...
async def predict_exoplanets(file: UploadFile = File(...)):
    ...
    try:
        ...
        results_df['PREDICTION'] = [label_encoder.classes_[p] for p in predictions]
        ...
        # 1. Full results CSV
        full_results_path = output_dir / f'full_results_{timestamp}.csv'
        ...
        confirmed_path = output_dir / f'confirmed_exoplanets_{timestamp}.csv'
        confirmed_df.to_csv(confirmed_path, index=False, encoding='utf-8')
        
        # 3. Summary report - FIXED: Added encoding='utf-8'
        processing_time = (datetime.now() - start_time).total_seconds()
        summary_text = create_summary_report(results_df, processing_time)
        summary_path = output_dir / f'summary_report_{timestamp}.txt'
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write(summary_text)
        
        # Create ZIP file with all results
        zip_path = output_dir / f'exoscan_results_{timestamp}.zip'
...
async def download_template():
    """Download a sample CSV template with required columns"""
    template_data = {
    ...
    template_path = Path('results') / 'koi_template.csv'
    template_path.parent.mkdir(exist_ok=True)
    template_df.to_csv(template_path, index=False)
    
    return FileResponse(
        path=template_path,
        filename='koi_template.csv',
        media_type='text/csv'
    )
